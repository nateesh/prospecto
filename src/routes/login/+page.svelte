...
>
	{#if isRegister}
		<Button
			...
			variant="outline"
			class="absolute right-4 top-4 md:right-8 md:top-8">Login</Button
		>
	{:else}
		<Button
			on:click={() => {
			...
			variant="outline"
			class="absolute right-4 top-4 md:right-8 md:top-8">Register</Button
		>
	{/if}
		...
		{#if isRegister && !forgotPass}
				...
				{:else if !isRegister && !forgotPass}
					<div class="flex flex-col space-y-2 text-center">
						...
						<p class="text-sm text-muted-foreground">Enter your details to log in.</p>
					</div>
				...
				{:else}
					...
					<div class="">
						<form action="?/forgot" method="POST" class="mx-auto">
